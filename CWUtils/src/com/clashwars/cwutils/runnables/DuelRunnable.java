...
public class DuelRunnable implements Runnable {
	...
	public void start() {
		...
		cwu.getConfig().addBackup(player1.getUniqueId(), backup1);
		cwu.getConfig().addBackup(player2.getUniqueId(), backup2);
		...
		taskID = cwu.getServer().getScheduler().scheduleSyncRepeatingTask(cwu.getPlugin(), new Runnable() {
		    int count = 3;
		    @Override
		    public void run() {
		    	if(count == 0) {
