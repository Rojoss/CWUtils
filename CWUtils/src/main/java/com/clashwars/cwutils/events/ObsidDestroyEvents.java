...
public class ObsidDestroyEvents implements Listener {
	...
	public void onBlockBreak(BlockBreakEvent event) {
		...
		if (cwu.getCfg().getStatus("blackpowder")) {
		...
		if (block.getType().equals(Material.TNT)) {
			if (block.hasMetadata("C4")) {
				...
				event.getBlock().removeMetadata("C4", cwu);
	...
	public void onBlockPlace(BlockPlaceEvent event) {
		...
		block.setMetadata("C4", new FixedMetadataValue(cwu, "C4"));
		return;
	}
	...
	public void onPrepareCraft(PrepareItemCraftEvent event) {
		...
		if (cwu.getCfg().getStatus("enderchestBlock")) {
	...
	private void igniteC4(final Block block) {
		...
		if (block.hasMetadata("C4")) {
			Bukkit.getScheduler().scheduleSyncDelayedTask(cwu, new Runnable() {
				public void run() {
					Chunk chunk = block.getChunk();
					for (Entity entity : chunk.getEntities()) {
						if (entity.getType() == EntityType.PRIMED_TNT) {
							if (entity.getLocation().distanceSquared(block.getLocation()) < 1) {
								entity.setMetadata("C4", new FixedMetadataValue(cwu, "C4"));
							}
						}
					}
				}
	...
	public void onEntityExplode(EntityExplodeEvent event) {
		if (!cwu.getCfg().getStatus("destroyableObsidian")) {
			return;
		}
