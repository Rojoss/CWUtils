...
public class TagRunnable implements Runnable {
	...
	public int getTimeLeft() {
		return (int) (((startTime + (cwu.getCfg().getTagTime() * 1000)) - System.currentTimeMillis()) / 1000);
	}
	...
	public void run() {
		try {
			while ((startTime + (cwu.getCfg().getTagTime() * 1000)) > System.currentTimeMillis() && cwu.isEnabled()) {
				Thread.sleep(100);
